...
fn single_handshake_fragment(version: ProtocolVersion, encrypted: bool) {
    ...
    let record = Record::from(test_handshake_message(version));
    ...
    let mut record_wire_bytes = Record {
        typ: records[0].typ,
    ...
    // The bounds of the deframed record should span the entire encoded record
    ...
    // Simulate decryption
    let mut deframed_record = deframed_record.into_plain_record();
    deframed_record.typ = ContentType::Handshake;
    let bounds = bounds.start + header_size..bounds.end;

    // Feed the record payload into the deframer. It should be a complete span.
    deframer
        .input_message_dtls(deframed_record, bounds)
        .unwrap();
    ...
    let reassembled_message = deframer.record(message_span, &record_wire_bytes);
...
fn multiple_handshake_fragment_in_order(
    ...
    for record_idx in 0..records.len() {
        ...
        if record_idx < records.len() - 1 {
            ...
            // We should get the whole handshake message out of the deframer
            let reassembled_handshake_message = deframer.record(message_span, &encoded_records);
            check_reassembled_message(
                record_idx,
                &record,
...
fn multiple_handshake_fragment_out_of_order_and_more_than_one_seq_1(version: ProtocolVersion) {
    let header_size = version.encrypted_header_len();
    let first_record = Record::from(test_handshake_message(version));
    let second_record = Record::from(test_handshake_message(version));
...
fn single_record_multiple_handshake_messages(version: ProtocolVersion) {
    ...
    for (message, expected_handshake_seq) in messages {
        ...
        let reassembled_handshake_message = deframer.record(message_span, &encoded_record);
        assert_eq!(
...
fn multiple_fragments_application_data(version: ProtocolVersion) {
    let encoded_first_record = Record::<Payload<'_>>::from(Message {
    ...
    let encoded_second_record = Record::<Payload<'_>>::from(Message {
        version: EncodableVersion::Legacy(version),
    ...
    for (encoded_record, expect_start, expect_end, expect_epoch, expect_record_seq) in [
        ...
        let deframed_record = deframed_record.into_plain_record();
        assert_eq!(deframed_record.typ, ContentType::ApplicationData);
        assert_eq!(deframed_record.version.version(), version);
        assert_eq!(epoch, expect_epoch);
        check_record_sequence(version, true, expect_record_seq, record_seq);
        assert_eq!(
            deframed_record.payload,
...
fn multiple_epochs_interleave_application_data_and_handshakes(version: ProtocolVersion) {
    ...
    let application_data = |content: usize| -> Record<Payload<'_>> {
        Record::<Payload<'_>>::from(Message {
            version: EncodableVersion::Legacy(version),
        ...
        records.clone().into_iter().enumerate()
    {
        ...
        // Simulate in-place decryption
        let mut deframed_record = deframed_record.into_plain_record();
        deframed_record.typ = expect_record.typ;
        let header_size = version.encrypted_header_len();
        let bounds = bounds.start + header_size..bounds.end;

        assert_eq!(deframed_record.payload, expect_record.payload.bytes());

        if deframed_record.typ == ContentType::Handshake {
            deframer
                .input_message_dtls(deframed_record, bounds)
                .unwrap();
        ...
        if let Some(span) = deframer.complete_span() {
            let reassembled_handshake_message = deframer.record(span, &wire_bytes);
            check_reassembled_handshake(
                record_idx,
                version,
...
fn check_reassembled_message(
    idx: usize,
    original_message: &Record<Payload<'_>>,
    reassembled_message: &Record<&[u8]>,
