...
impl Tls13State {
    pub(crate) fn handle<'m>(
        ...
        match self {
            Self::SkipRejectedEarlyData(e) => e.handle(input, output, transcript),
            ...
            Self::EarlyData(e) => e.handle(input, output, transcript),
...
mod client_hello {
    ...
    impl ServerHandler<Tls13CipherSuite> for Handler {
        fn handle_client_hello(
            ...
            let mut flight = HandshakeFlightTls13::new(transcript, st.protocol.is_dtls());
            let (
                Tls13Extensions {
                    certificate_types,
                    alpn_protocol,
    ...
    fn emit_server_hello(
        ...
        output.send_msg(Some(transcript), sh, false, false);
    ...
    fn emit_fake_ccs(output: &mut dyn Output<'_>) {
        ...
        output.send_msg(None, m, false, false);
    }
...
impl ExpectCompressedCertificate {
    fn handle_input(
        ...
        .handle_certificate(cert_payload, transcript)
    }
}
