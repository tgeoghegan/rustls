...
impl SendPath {
    ...
    fn send_key_update_request(&mut self, tls: &mut Vec<u8>) -> Result<(), Error> {
        ...
        let seq = self.outbound_handshake_seq();

        self.send_msg(
            Message::build_key_update_request(self.version(), seq),
            true,
            tls,
        );
...
impl SendOutput for SendPath {
    ...
    fn queue_requested_key_update(&mut self) {
        ...
        // TODO(DTLS): this is horrible but we have to fragment the key update
        let handshake_seq = self.outbound_handshake_seq();
        let message = Message::build_key_update_notify(self.version(), handshake_seq);
        let queued = if self.version() == ProtocolVersion::DTLSv1_3 {
    ...
    fn set_encrypter(
        ...
        self.encrypt_state
            .set_record_encrypter(encrypter, max_records, purpose, self.version());
    }
